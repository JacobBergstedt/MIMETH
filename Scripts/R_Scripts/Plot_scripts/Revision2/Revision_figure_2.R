...
balances <- readRDS("./Data/RData/Results/EWAS/M_values/Environment/EWAS_balance_effects_compositional_analysis_partition3_with_DC.rds") %>%
  group_by(Balance) %>%
...
ggsave("./Plots/Revision2_plots/Figure2/Fig2A_CMV_Total_v_Med.pdf", pltA, width = 6, height = 4, units = "cm")
ggsave("./Plots/Revision2_plots/Figure2/Fig2A_CMV_Total_v_Dir.pdf", pltB, width = 6, height = 4, units = "cm")
ggsave("./Plots/Revision2_plots/Figure2/Fig2A_CMV_Total_v_Dir_sign_Dir.pdf", pltC, width = 6, height = 4, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure2/scatter_CMV_med_v_CD8Diff_balance.pdf", pltB, width = 10, height = 10, units = "cm")

# C & D -----------------------------------------------------------------------

plt_frame_CD4 <- select(covs, SUBJID, CMV_serostatus, X_CD4_EMRA_of_total.panel1, Age, Sex, Smoking_status, Day_of_sampling) %>% 
  mutate(Age = Age / 50)
plt_frame_CD4 <- add_column(beta = meth[["cg15843262"]], plt_frame_CD4)


pltC1 <- plt_frame_CD4 %>%
  ggplot(aes(x = CMV_serostatus, y = X_CD4_EM_of_total.panel1, fill = CMV_serostatus)) +
...
ggsave("./Plots/Revision2_plots/Figure2/violin_CMV_v_CD4EM_m_values.pdf", pltC, width = 10, height = 10, units = "cm")


pltC2 <- plt_frame_CD4 %>%
  ggplot(aes(x = beta, y = X_CD4_EM_of_total.panel1)) +
...
ggsave("./Plots/Revision2_plots/Figure2/scatter_CD4EM_m_values.pdf", pltC2, width = 10, height = 10, units = "cm")


pltC <- pltC1 + pltC2 + plot_layout(widths = c(1, 3))
pltC[[1]] <- pltC[[1]] + labs(tag = "C")

ggsave("./Plots/Revision2_plots/Figure2/mediation_CD4_DNMT3A.pdf", pltC, width = 10, height = 10, units = "cm")
...
plt_frame_EMRA <- add_column(beta = meth[["cg15843262"]], plt_frame_EMRA)


pltD1 <- plt_frame_EMRA %>%
  ggplot(aes(x = CMV_serostatus, y = X_CD8_EMRA_of_total.panel1, fill = CMV_serostatus)) +
  geom_violin(color = "black", size = line_size_scatter) +
  ylim(c(0, 0.08)) +
  scale_fill_manual(values = c(reds[3], col_CMV)) +
  ylab("Prop CD8 EMRA") +
  ...
  theme(panel.grid = element_blank(),
        axis.text = element_text(color = "black"),
        legend.position = "none")
ggsave("./Plots/Revision2_plots/Figure2/violin_CMV_v_CD8EMRA_m_values.pdf", pltD1, width = 10, height = 10, units = "cm")


pltD2 <- plt_frame_EMRA %>%
  ggplot(aes(x = beta, y = X_CD8_EMRA_of_total.panel1)) +
  geom_point(size = point_size_scatter, col = col_cell) +
  ylim(c(0, 0.08)) +
  geom_smooth(method = "lm", size = line_size_scatter, se = FALSE, color = "black") +
  ylab(NULL) +
...
ggsave("./Plots/Revision2_plots/Figure2/scatter_CD8EMRA_m_values.pdf", pltD2, width = 10, height = 10, units = "cm")

pltD <- pltD1 + pltD2 + plot_layout(widths = c(1, 3))
pltD[[1]] <- pltD[[1]] + labs(tag = "D")

ggsave("./Plots/Revision2_plots/Figure2/mediation_CD8_DNMT3A.pdf", pltD, width = 10, height = 10, units = "cm")

# Manuscript CMV quants
...
ggsave("./Plots/Revision2_plots/Figure2/locus_zoomDNMT3A.pdf", pltE, width = 10, height = 10, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure2/TFBS_pos.pdf", pltF, width = 10, height = 10, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure2/TFBS_neg.pdf", pltG, width = 10, height = 10, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure2/Figure_2.png", plt, width = 172, height = 200, units = "mm", dpi = 500)
ggsave("./Plots/Revision2_plots/Figure2/Figure_2.pdf", plt, width = 172, height = 200, units = "mm")
