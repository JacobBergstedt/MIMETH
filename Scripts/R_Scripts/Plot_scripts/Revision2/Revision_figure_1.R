...
res_PCA <- readRDS("./Data/RData/Results/meth_PCA_regression_non_compositional.rds") %>% 
...
cell_list <- cell_list_16_cells

cells <- covariates %>% 
  select(SUBJID, all_of(cell_list))

cell_balances <- add_column(balance_preds(cells[-1], sbp = read_xlsx("./Data/Cell_subset_partitions_3_with_DC.xlsx")), 
...
ggsave("./Plots/Revision2_plots/Figure1/NR_sign_heatmap_sqrtscale.pdf", plt, width = 10, height = 10, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure1/scatter_Age_IDOL_v_16_cells.pdf", plt, width = 30, height = 20, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure1/scatter_CMV_IDOL_v_16_cells.pdf", plt, width = 10, height = 10, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure1/scatter_CMV_IDOL_v_CD4Diff_balance.pdf", pltF, width = 10, height = 10, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure1/scatter_CMV_16_cells_v_CD4Diff_balance.pdf", pltG, width = 10, height = 10, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure1/scatter_CMV_total_v_CD4Diff_balance.pdf", pltH, width = 10, height = 10, units = "cm")



plt <- pltF | pltG | pltH
ggsave("./Plots/Revision2_plots/Figure1/scatter_CMV_v_CD4Diff_balance.pdf", plt, width = 16, height = 16, units = "cm")

# PC plots ----------------------------------------------------------------

plt_frame <- meth_PC[c("SUBJID", "PC1", "PC2", "PC3")] %>% 
  inner_join(select(covariates, SUBJID, CMV_serostatus, Age, all_of(cell_list))) %>%
  mutate(CD8pheno = ifelse(X_CD8_naive_of_total.panel1 < median(X_CD8_naive_of_total.panel1), "Diff.", "Naive"))
...
ggsave("./Plots/Revision2_plots/Figure1/PC1_PC2_CMV.pdf", plt, height = 20, width = 30, units = "cm")

plt <- plt_frame %>% 
  ggplot() +
  geom_point(aes(x = X_CD4_EM_of_total.panel1, y = PC1, color = CMV_serostatus), size = point_size_scatter) +
  scale_color_manual(values = c(reds[3], col_CMV)) +
  xlab("Prop. CD4+ EM Cells") +
  ylab("Meth PC1") +
  geom_smooth(aes(x = X_CD4_EMRA_of_total.panel1, y = PC1), method = "lm", se = FALSE, size = line_size_blacks, color = "black") +
...
ggsave("./Plots/Revision2_plots/Figure1/PC1_CD4_CMV.pdf", plt, width = 10, height = 10, units = "cm")
...
pltE <- plt
ggsave("./Plots/Revision2_plots/Figure1/PC3_Age_CD8diff.pdf", plt, width = 10, height = 10, units = "cm")
...
ggsave("./Plots/Revision2_plots/Figure1/Figure_1.png", plt, width = 172, height = 172, units = "mm", dpi = 500)
ggsave("./Plots/Revision2_plots/Figure1/Figure_1.pdf", plt, width = 172, height = 172, units = "mm")
