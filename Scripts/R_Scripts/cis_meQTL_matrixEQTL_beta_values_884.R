...
covs <- get_covs_884()
meth_list <- parLapply(cl, 1:22, setup_meth, type = "Beta", covs = covs)
...
res <- mcmapply(run_local_meqtl_for_chr, meth_list, snp_list, meth_loc_list, snp_loc_list, MoreArgs = list(mm = meqtl_covariates), SIMPLIFY = FALSE, mc.cores = 11)
res <- bind_rows(res)

saveRDS(res, "./Data/RData/Results/MeQTL/cis_beta_values_884.rds")
