...
res_R2 <- collect_variance_explained_results()
res_R2_averaged <- res_R2 %>% group_by(Probe) %>% summarize(across(contains("effect"), list(mean, sd)))

res_R2_mean <- res_R2_averaged %>% select(Probe, contains("1"))
...
res_R2_gen_int <- readRDS("./Data/RData/Results/Proportion_of_variance_explained/prop_var_explained_genetic_interactions.rds") %>% 
  mutate(R2 = 100 * R2) %>% 
  group_by(Probe) %>% 
...
res_R2_mean <- res_R2_mean %>%
  ...
  ungroup() %>% 
  mutate(Top_pred = as.character(fct_recode(Top_pred, `Cell composition` = "Total_effect_cells", Intrinsic = "Total_effect_intrinsic", Genetic = "Total_effect_genetic", Exposures = "Total_effect_exposures")))

saveRDS(res_R2_mean, "./Data/RData/Results/Proportion_of_variance_explained/prop_var_explained_genetic_interactions_averaged_old.rds")
...
res_R2_mean <- readRDS("./Data/RData/Results/Proportion_of_variance_explained/prop_var_explained_genetic_interactions_averaged.rds")
