...
snps_per_probe <- snps_per_probe[names(meth)]
geno_sign <- snps[, unlist(snps_per_probe)]

snp_mat_list <- mclapply(snps_per_probe, function(x) snps[, x, drop = FALSE], mc.cores = 2)
n <- names(meth)[!names(meth) %in% names(snp_mat_list)]
names(snp_mat_list[n]) <- n
snp_mat_list <- snp_mat_list[names(meth)]
saveRDS(snp_mat_list, "./Data/RData/Genotypes/snp_mat_per_probe.rds")
