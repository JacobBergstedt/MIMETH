...
props <- readRDS("./Data/RData/Cells/proportions_of_total_outrm.rds") %>% 
  mutate_at(vars(-SUBJID), ~ . / 100) %>% 
  mutate(X_dendritic_cells.panel8 = X_pDC_of_total.panel8 + X_cDC1_of_total.panel8 + X_cDC3_of_total.panel8)

props_PCs <- get_20_PCs(props)
colnames(props_PCs) <- c("SUBJID", paste0("props_PC", 1:20))

# Include lineage panel ---------------------------------------------------
lineage <- readRDS("./Data/RData/Cells/proportions_panel5.rds") %>% 
  select(-X_mono_of_total.panel5)

# Include CD8 cells -------------------------------------------------------
props_CD8 <- readRDS("./Data/RData/Cells/proportions_CD8_cells.rds")
...
# Merge -------------------------------------------------------------------
covariates <- full_join(ecrf, props) %>% 
  full_join(props_PCs) %>% 
  full_join(idol_cells) %>% 
  full_join(houseman_cells) %>% 
  full_join(ancestry_pcs) %>%
  full_join(meth_batch) %>% 
  full_join(lineage, by = "SUBJID") %>% 
  full_join(props_CD8, by = "SUBJID")

covariates_all_samples <- covariates
covariates <- left_join(meth_ID, covariates)

# Write to file -----------------------------------------------------------
write(names(select(ecrf, -SUBJID, -Day_of_sampling, -Date_of_sampling, -Height)),
      "./Data/covariate_list.txt")

saveRDS(covariates, "./Data/RData/Environment/covariates.rds")
saveRDS(covariates_all_samples, "./Data/RData/Environment/covariates_all_samples.rds")
