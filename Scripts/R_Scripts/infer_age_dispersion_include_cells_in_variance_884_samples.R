...
fit_age_dispersion <- function(y, snp_mat, fm, mf) {
  ...
  mf <- na.omit(mf)
  null <- gamlss(fm, data = mf, sigma.formula = fm_sigma_null,
                 control = gamlss.control(trace = FALSE))
  alt <- update(null, formula. = fm_sigma_alt, what = "sigma")
...
fm_sigma_null <- as.formula(paste0(" ~ ", paste(get_15_props(), collapse = " + ")))
fm_sigma_alt <- update(fm_sigma_null, " ~ Age + .")
fm <- update(get_control_fm_15_cells(), y ~ . )

meth <- get_m_values()
covs <- get_covs_884() %>% 
  mutate(Age = Age / 50)

snp_mat_list <- readRDS("./Data/RData/Genotypes/snp_mat_per_probe.rds") %>% discard(is_null)
...
saveRDS(res, "./Data/RData/Results/Dispersion/res_dispersion_gamlss_variance_adjusted_for_cells_884.rds")